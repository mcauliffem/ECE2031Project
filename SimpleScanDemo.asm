...
Main:
	...
	;	add 90 and mod 360 to the angle value 			:)			;
	...
	;	if not a wall: 												;
	;		turn away from it, drive forward until another object	;
	;			is seen, then use above follower function if wall	;
	;;;;;;;;;;;;;;;;;END OF NEW CODE;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	;drive forward
	LOAD FSlow
	OUT  RVELCMD
...
ADWait:
	; turn the robot, using BOTH wheels only
	LOAD   FSlow
	OUT    RVELCMD
	; make left well move backwards at the same speed
	LOAD   RSlow
	OUT    LVELCMD
